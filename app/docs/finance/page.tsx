...
const TOC = [
  ...
  { id: "meta-marketing-cli", label: "Meta marketing CLI" },
  { id: "self-host-parity", label: "Self-host vs cloud" },
];
    ...
    <>
      <main className="docs-center">
        <article className="dc">
          ...
          <h1 className="dc__h1">Finance · ROAS · CAC</h1>
          <p className="dc__lede">
            ...
            you MRR, ARR, ROAS, CAC, and blended-CAC over any window. Stripe-mirrored data and
            agent-recorded data live in the same tables; you query them the same way.
          </p>
          ...
          <ul>
            ...
            <li>
              <code>subscription</code> — recurring revenue from a contact. Stores{" "}
            ...
            <li>
              ...
              bill), a <code>deal</code> (one-shot project bill), or standalone.
            </li>
            <li>
          ...
          <h2 className="dc__h2" id="business-profile">
            Business profile
          </h2>
          <p>
            Krabs ships <em>agent-first</em> — on first connect the agent runs a short kickoff
            to learn how you make money, then calls <code>businessProfile.set</code>. That single
            record shapes which primitives the agent will default to and how reporting is framed.
          </p>
          <pre className="dc__code">{`krabs account business-profile set \\
  --revenue-model hybrid \\
  --cadence monthly \\
  --currency USD \\
  --channels meta_ads,google_ads,referral \\
  --typical-contract-cents 20000`}</pre>
          <p>
            <strong>Revenue models:</strong> <code>recurring_saas</code> · <code>one_time</code> ·
            <code>hybrid</code> · <code>freelance</code> · <code>marketplace</code> ·
            <code>other</code>.{" "}
            <strong>Cadences:</strong> <code>weekly · monthly · quarterly · yearly · per_project ·
            mixed</code>.
          </p>
          <p>
            Read it back with <code>krabs account business-profile get</code> or{" "}
            <code>GET /v1/account/business-profile</code>. The kickoff runs once; re-call when
...
krabs finance expenses-by-category --from 2026-05-01T00:00:00Z --to 2026-05-31T23:59:59Z`}</pre>
          ...
          <ul>
            ...
            <li>
              ...
              paid). Null when no contacts converted.
            </li>
            <li>
          ...
          <pre className="dc__code">{`$ krabs finance funnel --from 2026-05-01T00:00:00Z --to 2026-05-31T23:59:59Z
{
  ...
  "blended_cac_cents":  89821
          ...
          <Callout tone="info" title="how &lsquo;new customers&rsquo; is computed">
            A new customer is a <code>contact</code> whose <code>status</code> moved to{" "}
            <code>customer</code> and whose <code>updatedAt</code> falls inside the window. The
            agent or your Stripe webhook handler is responsible for moving contacts to{" "}
            <code>customer</code> on first paid invoice. If you skip that step, CAC stays null.
          </Callout>
  ...
  krabs expense create \\
    ...
    --occurred-at "${'$'}{date}T00:00:00Z"
          ...
          <p>
            ...
            Self-host users get the same primitives, the same{" "}
            <code>/v1/finance/funnel</code> endpoint, and the same agent skill. The cloud version
            adds nothing here — it&apos;s the same code path.
          </p>
          <p>
            Stripe and Resend integrations are also part of the core. Connect them at{" "}
            <code>/dashboard/settings/integrations/&lt;provider&gt;</code> in either deployment.
          </p>
