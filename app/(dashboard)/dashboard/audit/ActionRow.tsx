  ...
  return (
    <div className="hover:bg-muted/50 transition-colors">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full text-left px-4 py-2.5 grid grid-cols-12 gap-3 items-center focus:outline-none focus:bg-muted/50"
      >
        <span className="col-span-1 text-muted-foreground tabular-nums">
          {time}
        </span>
        <span
        ...
        >
          {a.reversibility === "reversible" ? (
            ...
            <span className="inline-block px-1.5 py-0.5 rounded bg-muted text-muted-foreground text-[10px] uppercase tracking-wide">
              one-way
            </span>
          ) : (
      ...
      {open && (
        <div className="px-4 pb-3 pt-1 bg-muted/40 border-t border-border">
          ...
          <p className="k-eyebrow mb-1">metadata</p>
