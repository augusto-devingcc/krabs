...
const TABS: Record<TabKey, TabInfo> = {
  ...
  cli: {
    ...
    code: `$ krabs invoice.create --counterparty "Acme Inc" --amount 120000 --currency USD
{ "id": "inv_01J6Q…", "version": 1, "created": true }

$ krabs expense.create --amount 4200 --category hosting --dry-run
{ "plan": { "would_create": "exp_…", "amount": 4200 } }

$ krabs finance.report --from 2026-01-01 --to 2026-03-31
{ "income": 480000, "expenses": 152300, "net": 327700 }`,
  ...
  http: {
    ...
    code: `POST https://api.krabs.dev/v1/invoice.create
Authorization: Bearer $KRABS_TOKEN
Idempotency-Key: invoice-2401
Content-Type: application/json

{ "counterparty": "Acme Inc", "amount": 120000, "currency": "USD" }

→ 200 { "id": "inv_01J6Q…", "version": 1, "created": true }`,
  },
};
...
const FEATURES: Array<{ eyebrow: string; title: string; body: string }> = [
  ...
  {
    eyebrow: "undo",
    title: "Reversible where it counts.",
    body: "Account and key operations return an undo token — one call rewinds them. Agents experiment; mistakes do not become incidents.",
  },
  {
    eyebrow: "primitives",
    title: "Income, expenses, cashflow.",
    body: "Products, subscriptions, invoices, expenses — plus finance reporting. The primitives a one-person business actually tracks, and nothing it doesn't.",
  },
  {
    eyebrow: "safety",
  ...
  {
    eyebrow: "self-host",
    title: "No login. Your box, your data.",
    body: "Self-hosted, open source, MIT. Single account, no sign-up, no billing. SQLite on disk — back it up by copying a file.",
  },
  {
    eyebrow: "contract",
    title: "Self-described contract.",
    body: "GET /v1/schema returns the entire operation catalog. Your agent reads its own manual and stops asking you what's possible.",
  },
];
    ...
    <section className="mk-hero">
      ...
      <div className="mk-hero__inner">
        ...
        <p className="mk-hero__sub">
          {BRAND.productName} tracks income, expenses, and cashflow over one
          API with three equally first-class transports — MCP, HTTP, and CLI.
          Self-hosted, open source, no login. Built for solopreneurs whose
          agents do the work.
        </p>
        <div className="mk-hero__ctas">
          <Link href={BRAND.repo} className="mk-btn mk-btn--primary mk-btn--lg">
            Get it on GitHub
          </Link>
        ...
        <div className="mk-hero__terminal">
          ...
          <pre className="mk-term__body">
            ...
            <span className="mk-c-acc">--counterparty</span>
            {" "}
            <span className="mk-c-str">&quot;Acme Inc&quot;</span>
            {" "}
            ...
            <span className="mk-c-dim">→</span>
            {" "}
            <span className="mk-c-str">{`{ "id": "inv_01J6Q…", "version": 1, "created": true }`}</span>
            {"\n\n"}
            <span className="mk-c-dim">$</span>
            {" krabs expense.create "}
            <span className="mk-c-acc">--amount</span>
            {" 4200 "}
            <span className="mk-c-acc">--category</span>
            {" hosting "}
            <span className="mk-c-acc">--dry-run</span>
            {"\n"}
            <span className="mk-c-dim">→</span>
            {" plan ready · would_create "}
            <span className="mk-c-acc">exp_…</span>
            {"\n\n"}
            <span className="mk-c-dim">$</span>
            {" krabs finance.report "}
            <span className="mk-c-acc">--from</span>
            {" 2026-01-01"}
            {"\n"}
            <span className="mk-c-dim">→</span>
            {" income 480000 · expenses 152300 · net "}
            <span className="mk-c-acc">327700</span>
          </pre>
        </div>
      </div>
    </section>
    ...
    <section className="mk-fg" id="features">
      <div className="mk-fg__inner">
        <div className="mk-fg__lead">
          ...
          <h2 className="mk-h2">A finance backend that disappears so your agents can run.</h2>
        </div>
    ...
    <section className="mk-cta" id="install">
      <div className="mk-cta__inner">
        ...
        <p className="mk-cta__p">
          No demo call. No sign-up. Clone the repo, run setup, you have an API,
          MCP server, and CLI on localhost. Free and open source.
        </p>
